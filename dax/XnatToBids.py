                        ...
                        if scan not in data_type_l:
                            for scan_resources in os.listdir(os.path.join(sess_path, scan)):
                                for scan_file in os.listdir(os.path.join(sess_path, scan, scan_resources)):
                                    ...
                                    scan_path = '/projects/%s/subjects/%s/experiments/%s/scans/%s' % (project, subj, sess, scan.split('-x-')[0])
...
def bids_yaml(XNAT, project, scan_id, subj, res_dir, scan_file, uri, sess, nii_file):
    ...
    xnat_prov = yaml_create_json(XNAT, data_type, res_dir, scan_file, uri, project, scan_type, sess, is_json_present,
                                 nii_file, json_file)
    bids_fname = yaml_bids_filename(XNAT, data_type, scan_id, subj, sess, project, scan_file, scan_type)
    bids_fname_json = yaml_bids_filename(XNAT, data_type, scan_id, subj, sess, project, json_file, scan_type)
    ...
    data_type_dir = os.path.join(bids_dir, "sub-" + subj, "ses-" + sess, data_type)
...
def yaml_bids_filename(XNAT, data_type, scan_id, subj, sess, project, scan_file, scan_type):
    if data_type == "anat":
        ...
        bids_fname = "sub-" + subj + '_' + "ses-" + sess + '_task-' + task_type + '_acq-' + scan_id + '_run-01' \
                     + '_' + 'bold' + '.' + ".".join(scan_file.split('.')[1:])
        return bids_fname
    elif data_type == "dwi":
        if label == "BVEC":
            bids_fname = "sub-" + subj + '_' + "ses-" + sess + '_acq-' + scan_id + '_' + 'dwi' + '.' + 'bvec'

        elif label == "BVAL":
            bids_fname = "sub-" + subj + '_' + "ses-" + sess + '_acq-' + scan_id + '_' + 'dwi' + '.' + 'bval'

        else:
            bids_fname = "sub-" + subj + '_' + "ses-" + sess + '_acq-' + scan_id + '_' + 'dwi' + \
                         '.' + ".".join(scan_file.split('.')[1:])
        return bids_fname

    elif data_type == "fmap":
        bids_fname = "sub-" + subj + '_' + "ses-" + sess + '_acq-' + scan_id + '_bold' + \
                     '.' + ".".join(scan_file.split('.')[1:])
        return bids_fname


def yaml_create_json(XNAT, data_type, res_dir, scan_file, uri, project, scan_type, sess, is_json_present, nii_file,
                     json_file):
    if data_type != 'func':
        ...
        xnat_prov = yaml_func_json_sidecar(XNAT, data_type, res_dir, scan_file, uri, project, scan_type, nii_file,
                                           is_json_present, sess, json_file)
        # print xnat_prov
    return xnat_prov


def yaml_func_json_sidecar(XNAT, data_type, res_dir, scan_file, uri, project, scan_type, nii_file, is_json_present,
                           sess, json_file):
    xnat_prov = None
